...
def _score_from_findings(findings: List[Finding]) -> int:
    ...
    penalties = {"fail": 1.0, "warning": 0.4, "unknown": 0.2, "pass": 0.0}
...
def generate_report(audit_data: Dict, language: str = "en") -> str:
    ...
    # Priority list: fail/warning only, sorted by severity then status
    ...
    if language == "zh":
        return _build_report_zh(url, date, lang_detected, scores, all_module_findings, priority_findings, render_fn)
    return _build_report_en(url, date, lang_detected, scores, all_module_findings, priority_findings, render_fn)


def _build_report_en(url, date, lang_detected, scores, module_findings, priority_findings, render_fn) -> str:
...
*Generated by [google-seo-audit](https://github.com/google-seo-audit)*
...
def _build_report_zh(url, date, lang_detected, scores, module_findings, priority_findings, render_fn) -> str:
    status_label = {mod: _status_label(s) for mod, s in scores.items()}
    summary_rows = "\n".join(
        f"| {mod} | {STATUS_EMOJI.get(status_label[mod], '')} | {scores[mod]}/100 |"
        for mod in scores
...
*由 [google-seo-audit](https://github.com/google-seo-audit) 生成*
